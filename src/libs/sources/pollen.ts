...
export async function getPollen(lat: number, lon: number): Promise<PollenData | null> {
  if (!config.pollen.apiKey) return null;

  const key = `pollen:v1:${lat.toFixed(2)},${lon.toFixed(2)}`;
  return getOrSet(key, { ttlSeconds: config.cache.pollenTtl }, async () => {
    const url =
      "https://pollen.googleapis.com/v1/forecast:lookup" +
      `?location.latitude=${lat}&location.longitude=${lon}` +
      `&days=1&plantsDescription=false&languageCode=en&key=${config.pollen.apiKey}`;
